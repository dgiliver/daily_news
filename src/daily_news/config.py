...
class Settings(BaseSettings):
    ...
    db_path: Path = Field(
        default=Path("data/news_archive.db"), description="SQLite database path"
    )
    ...
    email_recipients: str = Field(
        default="", description="Comma-separated email recipients"
    )

    # SMS Configuration
    sms_recipients: str = Field(
        default="", description="Comma-separated phone numbers"
    )
    sms_carrier_gateway: str = Field(
        default="txt.att.net", description="SMS carrier gateway"
    )
    ...
    digest_story_count: int = Field(
        default=15, description="Number of stories in email digest"
    )
    sms_headline_count: int = Field(
        default=5, description="Number of headlines in SMS"
    )
    ...
    ranking_batch_size: int = Field(
        default=50, description="Articles per Claude ranking call"
    )
    ...
    enable_deduplication: bool = Field(
        default=True, description="Enable duplicate article removal"
    )
